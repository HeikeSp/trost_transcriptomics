...
source("../functions/func_edgeR_dge.R")
source("../functions/func_pageman_res.R")
source("../functions/func_merge_sig_res_mapman.R")
source("../functions/func_edgeR_top.R")
source("../functions/func_intersecting_dge.R")
source("../functions/func_final_DGE_list.R")
source("../functions/func_order_by_MapMan_BinCode.R")
source("../functions/func_create_type.R")
source("../functions/func_combine_two_vectors.R")
source("../functions/func_combine_two_final_DGE_lists.R")
```
...
pdf("figures/venn_cultivation_condition_down.pdf", width = 4, height = 4)
grid.draw(venn.plot)
dev.off()
```
...
pdf("figures/venn_cultivation_condition_up.pdf", width = 4, height = 4)
grid.draw(venn.plot)
dev.off()
```
...
pdf("figures/venn_cultivation_control_tol_up.pdf", width = 4, height = 4)
grid.draw(venn.plot)
dev.off()
...
pdf("figures/venn_cultivation_control_tol_down.pdf", width = 4, height = 4)
grid.draw(venn.plot)
dev.off()
```
...
pdf("figures/venn_cultivation_stress_tol_up.pdf", width = 4, height = 4)
grid.draw(venn.plot)
dev.off()
```
...
pdf("figures/venn_cultivation_stress_tol_down.pdf", width = 4, height = 4)
grid.draw(venn.plot)
dev.off()
```
...
# save as: "venn_all_conditions_tol_up.pdf"
```
...
# save as: "venn_all_conditions_tol_down.pdf"
```
...
# genes from MapMan Bin stress.abiotic.drought --> Code: 20.2.3
```{r genes from stress.abiotic.drought}
mapman_drought_stress <- merge_mapman_pgsc_dmg[which(grepl("20.2.3", merge_mapman_pgsc_dmg$BINCODE)),]
write.table(mapman_drought_stress, "output/mapman_drought_stress.txt", sep = "\t", row.names = F)


# heat stress
mapman_heat_stress <- merge_mapman_pgsc_dmg[which(grepl("20.2.1", merge_mapman_pgsc_dmg$BINCODE)),]
write.table(mapman_heat_stress, "output/mapman_heat_stress.txt", sep = "\t", row.names = F)
```
