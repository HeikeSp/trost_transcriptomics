...
assoc_mapman_true <- subset(assoc_mapman, assoc_mapman$TYPE==TRUE)
...
write.table(res_greenhouse_control_pageman_mapman_T,
            "output/res_greenhouse_control_pageman_mapman_T.txt",
            sep="\t", row.names=F, quote=F)
```
...
# select columns: pgsc_dmp, logFC, pvalue, FDR
...
#### merge Pageman res to MapMan bins field
```{r merge Pageman res to MapMan bins field (control vs. stress)}
# only significant DGE: dir must be -1 or +1 (not 0)!
# tolerant
res_field_tolerant_pageman_mapman <- merge(
  res_field_tolerant_pageman[which(res_field_tolerant_pageman$dir%in%c(-1,1)),], 
  assoc_mapman, by.x = "pgsc_dmp", by.y = "IDENTIFIER")
# sensitive
res_field_sensitive_pageman_mapman <- merge(
  res_field_sensitive_pageman[which(res_field_sensitive_pageman$dir%in%c(-1,1)),], 
  assoc_mapman, by.x = "pgsc_dmp", by.y = "IDENTIFIER")
...
#### merge Pageman res to MapMan bins greenhouse
```{r merge Pageman res to MapMan bins greenhouse (control vs. stress)}
# only significant DGE: dir must be -1 or +1 (not 0)!
# tolerant
res_greenhouse_tolerant_pageman_mapman <- merge(
  res_greenhouse_tolerant_pageman[which(res_greenhouse_tolerant_pageman$dir%in%c(-1,1)),], 
  assoc_mapman, by.x = "pgsc_dmp", by.y = "IDENTIFIER")

# sensitive
res_greenhouse_sensitive_pageman_mapman <- merge(
  res_greenhouse_sensitive_pageman[which(res_greenhouse_sensitive_pageman$dir%in%c(-1,1)),],
  assoc_mapman, by.x = "pgsc_dmp", by.y = "IDENTIFIER")
