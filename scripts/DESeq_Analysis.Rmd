...
samplelist_ordered$sample_name <- as.character(samplelist_ordered$sample_name)
rownames(samplelist_ordered) <- samplelist_ordered$sample_name
...
The two functions, ``rlog`` and ``varianceStabilizingTransformation``, have an argument ``blind``, for whether the transformation should be blind to the sample information specified by the design formula. When ``blind`` equals ``TRUE`` (the default), the functions will re-estimate the dispersions using only an intercept (design formula ∼ 1). This setting should be used in order to compare samples in a manner wholly unbiased by the information about experimental groups, for example to perform sample QA (quality assurance) as demonstrated below.
...
# variance Stabilizing Transformation (vst) per cultivation type
vsd_greenhouse <- getVarianceStabilizedData(dds_greenhouse)
vsd_field <- getVarianceStabilizedData(dds_field)
dim(vsd_field)
vsd_all <- getVarianceStabilizedData(dds)
...
# Heatmap of genes from mapman bin stress.abiotic.drought
```{r heatmap of genes from mapman bin stress.abiotic.drought}
rld_assay <- assay(rld)
dim(rld_assay)

mapman_drought_stress <- read.table("output/mapman_drought_stress.txt", sep = "\t", header = T)
#mapman_drought_stress <- read.table("output/mapman_heat_stress.txt", sep = "\t", header = T)

# number of unique genes that are annotated as drought stress related in mapman
length(unique(mapman_drought_stress$pgsc_dmg))  # 72

# get rows where those genes are in filtered reads table
mapman_drought_stress_idx <- which(rownames(rld_assay) %in% mapman_drought_stress$pgsc_dmg)
length(mapman_drought_stress_idx) # 62

# there are 10 genes not present in the dataset because they were filtered out at the beginning (to few reads)
setdiff(mapman_drought_stress$pgsc_dmg, rownames(rld_assay))

rld_mapman_drought_stress <- rld_assay[mapman_drought_stress_idx,]
colnames(rld_mapman_drought_stress) <- samplelist_ordered$sample_name

cols_treatment_heatmap <- rep(cols_treatment[1], ncol(rld_mapman_drought_stress))
cols_treatment_heatmap[samplelist_ordered$condition=="drought stress"] <- cols_treatment[2]

cols_treatment_heatmap <- rep(cols_treatment_sample_time[1], ncol(rld_mapman_drought_stress))
cols_treatment_heatmap[samplelist_ordered$cultivation_condition=="field.drought stress"] <- cols_treatment_sample_time[2]
cols_treatment_heatmap[samplelist_ordered$cultivation_condition=="greenhouse.control"] <- cols_treatment_sample_time[3]
cols_treatment_heatmap[samplelist_ordered$cultivation_condition=="greenhouse.drought stress"] <- cols_treatment_sample_time[4]

pdf("figures/heatmap_rld_mapman_drought_stress_bin.pdf", 12, 10)
heatmap.2(rld_mapman_drought_stress, 
          scale = "row", 
          trace = "none", 
          dendrogram = "both", 
          margins = c(5, 10),
          ColSideColors = cols_treatment_heatmap,
          col = colorRampPalette( rev(brewer.pal(9, "RdBu")) )(255) )
dev.off()

# only greenhouse
rld_greenhouse_assay <- assay(rld_greenhouse)
rld_greenhouse_mapman_drought_stress <- rld_greenhouse_assay[mapman_drought_stress_idx,]
colnames(rld_greenhouse_mapman_drought_stress) <- samplelist_ordered_g$sample_name
cols_treatment_heatmap_g <- rep(cols_treatment[1], ncol(rld_greenhouse_mapman_drought_stress))
cols_treatment_heatmap_g[samplelist_ordered_g$condition=="drought stress"] <- cols_treatment[2]

#my_breaks <- c(seq(-4,-1.1,length=100),seq(-1,1,length=100),seq(1.1,4,length=100))
my_palette <- colorRampPalette( rev(brewer.pal(9, "RdBu")) )(299)

pdf("figures/heatmap_rld_greenhouse_mapman_drought_stress_bin.pdf", 10, 10)
heatmap.2(rld_greenhouse_mapman_drought_stress, 
          scale = "row", 
          trace = "none", 
          dendrogram = "both", 
          margins = c(5, 10),
          ColSideColors = cols_treatment_heatmap_g,
          col = my_palette)
          #breaks = my_breaks)
dev.off()

# only field
rld_field_assay <- assay(rld_field)
rld_field_mapman_drought_stress <- rld_field_assay[mapman_drought_stress_idx,]
colnames(rld_field_mapman_drought_stress) <- samplelist_ordered_g$sample_name
cols_treatment_heatmap_f <- rep(cols_treatment[1], ncol(rld_field_mapman_drought_stress))
cols_treatment_heatmap_f[samplelist_ordered_f$condition=="drought stress"] <- cols_treatment[2]

#my_breaks <- c(seq(-4,-1.1,length=100),seq(-1,1,length=100),seq(1.1,4,length=100))
my_palette <- colorRampPalette( rev(brewer.pal(9, "RdBu")) )(299)

pdf("figures/heatmap_rld_field_mapman_drought_stress_bin.pdf", 10, 10)
heatmap.2(rld_field_mapman_drought_stress, 
          scale = "row", 
          trace = "none", 
          dendrogram = "both", 
          margins = c(5, 10),
          ColSideColors = cols_treatment_heatmap_f,
          col = my_palette)
          #breaks = my_breaks)
dev.off()
```
...
# selected genes for qRT-PCR
```{r selected genes for qRT-PCR}
sel_genes <- c("PGSC0003DMG400006743","PGSC0003DMG400014310","PGSC0003DMG400011073","PGSC0003DMG400024160",
               "PGSC0003DMG400002993","PGSC0003DMG400013605","PGSC0003DMG400024120","PGSC0003DMG400012619",
               "PGSC0003DMG400017098","PGSC0003DMG400000066","PGSC0003DMG400018431","PGSC0003DMG400022343",
               "PGSC0003DMG400010258","PGSC0003DMG400028600","PGSC0003DMG400000478","PGSC0003DMG400002663",
               "PGSC0003DMG400018569","PGSC0003DMG400012988","PGSC0003DMG400012826","PGSC0003DMG401033888",
               "PGSC0003DMG400005247","PGSC0003DMG400007816","PGSC0003DMG400010507","PGSC0003DMG400012305",
               "PGSC0003DMG400019671","PGSC0003DMG401022350","PGSC0003DMG400012020","PGSC0003DMG400009706",
               "PGSC0003DMG400011033","PGSC0003DMG401008889","PGSC0003DMG400029201","PGSC0003DMG400004038",
               "PGSC0003DMG400013010","PGSC0003DMG402029631","PGSC0003DMG400007458","PGSC0003DMG400012017",
               "PGSC0003DMG400014702","PGSC0003DMG400028358","PGSC0003DMG400010050","PGSC0003DMG400017157",
               "PGSC0003DMG400003358","PGSC0003DMG400017203","PGSC0003DMG400007179","PGSC0003DMG400016996")

sel_genes_up <- sel_genes[1:24]
sel_genes_down <- sel_genes[25:44]
...
mat_breaks <- quantile_breaks(assay(rld_greenhouse)[ sel_genes, ], n = 11)
col_heatmap = colorRampPalette(rev(brewer.pal(n = 9, name = "RdBu")))(length(mat_breaks))

heatmap.2( assay(rld_greenhouse)[ sel_genes, ], scale = "row",
           trace = "none", dendrogram = "column",
           col = col_heatmap)
...
heatmap.2( assay(rld_greenhouse)[ sel_genes_up, ], scale = "row",
           trace = "none", dendrogram = "column", 
           col = colorRampPalette( rev(brewer.pal(9, "RdBu")) )(255))
           #col = col_heatmap)

heatmap.2( assay(rld_greenhouse)[ sel_genes_down, ], scale = "row",
           trace = "none", dendrogram = "column", 
           #col = colorRampPalette( rev(brewer.pal(9, "RdBu")) )(255))
           col = col_heatmap)

# pheatmap(assay(rld_greenhouse)[sel_genes_up,], scale = "row", color = col_heatmap,
#          annotation_col = data.frame(treatment = as.character(samplelist_ordered_g$condition)),
#          annotation_colors = list(treatment = cols_treatment))

heatmaply( assay(rld_greenhouse)[ sel_genes_up, ], scale = "row", col = col_heatmap)
heatmaply( assay(rld_greenhouse)[ sel_genes_down, ], scale = "row", col = col_heatmap)

heatmap.2( assay(rld_greenhouse)[ sel_genes_down, ], scale = "row",
           trace = "none", dendrogram = "column",
           col = colorRampPalette( rev(brewer.pal(9, "RdBu")) )(255))

boxplot(assay(rld_greenhouse)["PGSC0003DMG400006743",] ~ samplelist_ordered_g$condition)
boxplot(assay(rld_greenhouse)["PGSC0003DMG400006743",] ~ samplelist_ordered_g$condition * samplelist_ordered_g$cultivar)

# PGSC0003DMG400007179
boxplot(assay(rld_greenhouse)["PGSC0003DMG400007179",] ~ samplelist_ordered_g$condition * samplelist_ordered_g$cultivar)
```
