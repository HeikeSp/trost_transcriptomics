...
getwd()
setwd("repos/trost_transcriptomics")
```
...
source("../libpurzel/func_cor_mtest.R")
source("../libpurzel/func_specific_genes.R")
```
...
### Modify samplelist
```{r modify samplelist}
# interaction of two factors: cultivar x condition
levels(interaction(samplelist_ordered$cultivar, samplelist_ordered$condition))

# interaction of two factors: cultivation x condition
...
samplelist_ordered <- cbind(samplelist_ordered, 
                            trial, 
...
# change colnames of samplelist
colnames(samplelist_ordered) <- c("sample_number", "sample_name", "trial_number", "trial_name", 
                                  "cultivation", "cultivar", "condition", "tolerance",
                                  "libsize_raw", "libsize_mapped", "trial", 
                                  "cultivar_condition","cultivation_condition")
...
write.table(samplelist_ordered, "output/samplelist_ordered2.txt", sep="\t")
```
...
func_5pairs_plot_sym(genes_ppca_greenhouse_log2_res, factors = samplelist_greenhouse, 
                     color_factor = "cultivar", c(19,17), "condition", symbol_size=2)
...
identified_transcripts_per_sample <- apply(fpkm_all, 2, function(x) length(which(x>0)))
summary(identified_transcripts_per_sample)

high_conf_transcripts_per_sample <- apply(fpkm_all, 2, function(x) length(which(x>1)))
summary(high_conf_transcripts_per_sample)
high_conf_transcripts_per_sample[1]

write.table(cbind(identified_transcripts_per_sample, high_conf_transcripts_per_sample), "../output/number_of_transcripts.txt", sep="\t")

identified_transcripts_per_sample_idx <- apply(fpkm_all, 2, function(x) which(x>0))
high_conf_transcripts_per_sample_idx <- apply(fpkm_all, 2, function(x) which(x>1))
...
# union
length(Reduce(union, identified_transcripts_per_sample_idx))
length(Reduce(union, high_conf_transcripts_per_sample_idx))
...
```{r venn diagram of specific genes}
w=list(
  "Alegria" = field_a,
  "Desiree" = field_d,
  "Milva" = field_m,
...
venn.plot <- venn.diagram(
  x = w,  
  #filename = "../figures/all_field_venn_all_cultivars.png",
  filename=NULL,
  col = "black",
...
write.table(fpkm_fc_field_sig_dmp, "../output/fpkm_fc_field_sig_dmp.txt", row.names=F, quote=F, sep="\t")
...
write.table(fpkm_fc_greenhouse_sig_dmp, "../output/fpkm_fc_greenhouse_sig_dmp.txt", row.names=F, quote=F, sep="\t")
